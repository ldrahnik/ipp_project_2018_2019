...
INTERPRET_TASK_TESTS_LOG_DIR=$2
INTERPRET_TASK=$SCRIPTS_DIR/$INTERPRET_TASK_NAME

for TEST_NAME in stack_test write_test; do
    $INTERPRET_TASK_INTERPRET $INTERPRET_TASK.$INTERPRET_TASK_EXTENSION < ${LOCAL_IN_PATH}$INTERPRET_TASK_TESTS_DIR$TEST_NAME.src --input ${LOCAL_IN_PATH}$INTERPRET_TASK_TESTS_DIR$TEST_NAME.in > ${INTERPRET_TASK_TESTS_LOG_DIR}$TEST_NAME.out 2>/dev/null
