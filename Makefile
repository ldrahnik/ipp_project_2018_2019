...
INTERPRET_TASK_TESTS_OUTPUT_DIR				= $(PWD)/tests/supplementary-tests/interpret-only/log/
PARSER_TASK_TESTS_REF_OUTPUT_DIR 			= $(PWD)/tests/supplementary-tests/parse-only/
INTERPRET_TASK_TESTS_REF_OUTPUT_DIR			= $(PWD)/tests/supplementary-tests/interpret-only/
...
test:
	...
	cd $(TESTS_DIR) && bash $(SUPPLEMENTARY_TESTS_DIFF_SCRIPT) $(JEXAMXML) $(JEXAMXML_TMP_FILE) $(JEXAMXML_OPTIONS_FILE) $(PARSER_TASK_TESTS_OUTPUT_DIR) $(PARSER_TASK_TESTS_REF_OUTPUT_DIR)
